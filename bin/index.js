...
const { Argument } = __nccwpck_require__(856);
const { Command } = __nccwpck_require__(340);
const { CommanderError, InvalidArgumentError } = __nccwpck_require__(297);
const { Help } = __nccwpck_require__(193);
const { Option } = __nccwpck_require__(224);
...
const { InvalidArgumentError } = __nccwpck_require__(297);
...
const { Argument, humanReadableArgName } = __nccwpck_require__(856);
const { CommanderError } = __nccwpck_require__(297);
const { Help } = __nccwpck_require__(193);
const { Option, splitOptionFlags, DualOptions } = __nccwpck_require__(224);
const { suggestSimilar } = __nccwpck_require__(469);
...
const { humanReadableArgName } = __nccwpck_require__(856);
...
;// CONCATENATED MODULE: ./dist/actionArgs.js
// Copyright (c) 2022 Upwave, All Rights Reserved
...
var ActionMode;
(function (ActionMode) {
    ActionMode[ActionMode["LIST"] = 0] = "LIST";
...
var commander = __nccwpck_require__(662);
...
var external_util_ = __nccwpck_require__(849);
...
class Tag {
     ...
     */
    toString() {
        return (0,external_util_.format)('%s%s.%s%s.%s', this.prefix, this.year, leadingZero(this.month), leadingZero(this.day), leadingZero(this.build));
    }
}
/**
 ...
 */
function create(priorTag, prefix) {
    // start with current date and build 1
    const currentDate = new Date(Date.now());
    let tagDate = currentDate;
    let buildNumber = 1;
    ...
    if (tagParts) {
        ...
        tagDate = new Date(Number.parseInt(tagParts[1]), Number.parseInt(tagParts[2], 10) - 1, Number.parseInt(tagParts[3], 10));
...
function leadingZero(val) {
    ...
    return (0,external_util_.format)(formatString, val);
}

;// CONCATENATED MODULE: ./dist/tagger.js
// Copyright (c) 2021 Upwave, All Rights Reserved
...
function getNextTag(currentTag, prefix) {
...
(function (Action) {
    ...
    function setupCommand(program) {
        const prefixOption = new Option('-p, --prefix <prefix>', 'optional tag prefix');
    ...
    Action.setupCommand = setupCommand;
    /**
     * Runs the action.
     *
     * @param args
